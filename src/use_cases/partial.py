...
def find_partials(ent1: List[EntitiesContainer], ent2: List[EntitiesContainer]):
    report = ""
    for e1 in ent1:
...
def report(p):
    files_entities = collect_files_and_json_entities(p)
    docs_entities = collect_docs_entities(p.documentation)
    return find_partials(files_entities, docs_entities)